...
function composerHasAttackShape(words=selectedWordData()){return words.some(w=>w.category==='shape'&&w.attackLike)}
...
function renderComposer(){renderPrefixes();const words=selectedWordData(),hasAttackShape=composerHasAttackShape(words),hasBaseAttack=composerHasBaseAttack(words);const c=getCharacter($('#composerCharacter')?.value),w=hasBaseAttack?selectedWeapon():null;const prefixKeys=Object.keys(PREFIXES).filter(k=>selectedPrefixes.has(k));const invalidPrefixes=prefixKeys.length>0&&!composerAllowsPrefixes();const hasAction=hasBaseAttack||words.length>0;const valid=hasAction&&!invalidPrefixes;const isSmite=hasBaseAttack&&words.length===1&&words[0].category==='source';const wordAp=words.length-(isSmite?1:0);const ap=(hasBaseAttack?1:0)+prefixKeys.length+wordAp;const basePotential=hasBaseAttack?BASE_ATTACK_POTENTIAL:0;const wordPotential=words.filter(x=>x.category==='source'||x.category==='shape').length;const prefixPotential=prefixKeys.reduce((s,k)=>s+PREFIXES[k].potential,0);const potential=basePotential+wordPotential+prefixPotential;const mana=words.reduce((s,x)=>s+(x.mana||0),0),dust=words.reduce((s,x)=>s+(x.dust||0),0);
  ...
  const readable=x=>t(x.id==='thunder'&&x.category==='source'?'Lightning':x.name),sources=words.filter(x=>x.category==='source');
  const parts=prefixKeys.map(k=>t(PREFIXES[k].label));if(hasBaseAttack){parts.push(w?t('Weapon Attack ({weapon})',{weapon:t(w.name)}):t('Attack'));if(sources.length)parts.push(t('of {sources}',{sources:sources.map(readable).join('/')}));parts.push(...words.filter(x=>x.category!=='source').map(readable))}else parts.push(...words.map(readable));
  $('#actionName').textContent=invalidPrefixes?t('Choose an attack for these prefixes'):parts.join(' ')||t('Choose components');
  const incantation=[...prefixKeys.map(k=>PREFIXES[k].word),...words.map(x=>x.word)].join(' ');$('#actionIncantation').hidden=!valid||!incantation;$('#actionIncantation').textContent=valid&&incantation?t('Alternative incantation (tracker flavor, not canonical): {name}',{name:incantation}):'';
  let accuracy='—';
  if(w){accuracy=c?`${w.accuracy} d${c.current[w.accuracy]}`:w.accuracy}
  else if(!hasBaseAttack&&words.length){const spellAttr=c&&idx(c.current.AUR)>idx(c.current.INT)?'AUR':'INT';accuracy=c?`${spellAttr} d${c.current[spellAttr]}`:t('INT or AUR, whichever is higher')}
